...
void ModelBram::setup(string path, float fboWidth, float fboHeight) {
...
void ModelBram::update(float speed) {
	vector<ofVec3f>& vertices = vboMesh.getVertices();
	vector<ofIndexType>& indices = vboMesh.getIndices();
	vector<ofVec3f> normals = vboMesh.getNormals();
...
void ModelBram::draw() {
	...
	float hueStart = fmod(ofGetElapsedTimef() * 12, 255);

	ofClear(ofColor::fromHsb(360, ofGetScreenWidth(), ofGetScreenHeight()));
	...
	for (ofVec3f& v : vboMesh.getVertices()) {
		...
		float radius = ofMap(x, -1, 1, 0.02, 0.05);
		radius += ofRandom(0.03);
		ofDrawSphere((v * 30) + ofRandom(0, 0.001), radius);
...
void ModelBram::drawInFbo() {
	fbo.begin();
	draw();
	fbo.end();
}

void ModelBram::drawFbo(float x, float y) {
	fbo.draw(x, y);
}
