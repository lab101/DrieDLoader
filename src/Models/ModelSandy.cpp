...
void ModelSandy::setup(string path, float fboWidth, float fboHeight) {
...
void ModelSandy::update(float speed) {
...
void ModelSandy::drawInFbo() {
    fbo.begin();
    draw();
    fbo.end();
}

void ModelSandy::drawFbo(float x, float y) {
    fbo.draw(x, y);
}
